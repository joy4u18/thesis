...
%             : The script is explicitly for 5ms signals only
...
cd('C:\Users\joy4u\OneDrive\Joy\Research\Thesis\RECORDINGS\Exactduration_callibration\anechoic\5ms\right_ear')
...
for i=1:length(wavfile)
    ...
    T_input=T_input(1:round(T_fs*0.020));
...
if (size(T_outputamp,1)==8)
    ...
    %%%% Note that 5ms anechoic signals have only 9 versions
    %%%% so compensated by copying version 7 as version 6 for that condition %%%
    ...
    latexinput.tableCaption = 'RMS amplitude values of 10 versions for the right ear recordings in the anechoic chamber with  5ms duration signal';
    latexinput.tableLabel = 'RMS_anechoic_5ms_right';
    ...
    % latexinput.tableColumnAlignment = 'c';
    % latexinput.tableBorders = 1;
    % latexinput.tableCaption = 'Mean of the RMS amplitude values of  10 versions for the right ear recordings in the anechoic chamber with  5ms duration signal';
    % latexinput.tableLabel = 'RMS_mean_anechoic_5ms_right';
    % latexinput.makeCompleteLatexDocument = 0;
    % latexoutput=latexTable(latexinput);
