...
# load the data
SThdData <- read_excel("E:/GitHub/thesis/matlab/statistics/MeanSdIFFthdRanalysis.xlsx")
...
# plot
obj1 <- ggplot(SThdData, aes(y=Threshold, x=Observation,  shape = Group, color =  Group, group = Group)) +geom_point(  size = 5) + scale_color_discrete(name="", labels = c("Blind", "Sighted")) + scale_shape_discrete(name ="", labels = c("Blind", "Sighted"))
...
obj3 <- obj2 +labs(x="", y="Sharpness Threshold (acum)")
...
obj10 <- obj9  + scale_y_continuous(breaks = seq(0,2,0.5)) +scale_x_discrete(breaks = seq(0,2,1))
...
obj12 <- obj11 + theme(legend.spacing.x =  unit(1,"line"), legend.title = element_blank(), legend.position = c(0.9, 0.85), legend.background = element_rect(fill="lightblue",
