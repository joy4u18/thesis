...
# load the data
LThdData <- read_excel("E:/GitHub/thesis/matlab/statistics/MeanLDiffthdRanalysis.xlsx")
...
# plot
obj1 <- ggplot(LThdData, aes(y=Threshold, x=Observation,  shape = Group, color =  Group, group = Group)) +geom_point(  size = 5) + scale_color_discrete(name="", labels = c("Blind", "Sighted")) + scale_shape_discrete(name ="", labels = c("Blind", "Sighted"))
...
obj3 <- obj2 +labs(x="", y="Loudness Threshold (sone)")
...
obj10 <- obj9  + scale_y_continuous(breaks = seq(0,60,10)) +scale_x_discrete(breaks = seq(0,2,1))
...
obj12 <- obj11 + theme(legend.spacing.x =  unit(1,"line"), legend.title = element_blank(), legend.position = c(0.85, 0.85), legend.background = element_rect(fill="lightblue",
