...
# load the data
pitchData <- read_excel("E:/GitHub/thesis/matlab/auditorymodels/Loudness_meanresults/PS.xlsx")
...
(obj1 <- ggplot(data=pitchData,
                aes( y=Pitch,x=factor(Object_Distance), color=factor(Duration),
                     shape=factor(Duration), group=factor(Duration))) +geom_point(size=3, position = position_dodge(width = 0.4)) + geom_path(size=1, position = position_dodge(width = 0.4)) + geom_errorbar(aes(ymin = Pitch - SEM, 
                    ymax = Pitch + SEM),
                width = 1,size = 1, position = "dodge", colour = "black")) 


obj2 <- obj1 + facet_grid(cols = vars(Room) )
obj3 <- obj2 +labs(x="Distance (m)", y="Pitch Strength (autocorrelation index)")
...
obj9 + scale_y_continuous(breaks = seq(0,10,1))
