...
% figure(figure_handle);   % VJ 2015 this is the actual code but commented it so that the plot works even after closing the figure window
figure(handles.info.current_figure);
clf
...
% 			sig=getpart(sig,start_time,stop_time);
            ...
            if hand_scaling == 1
             ...
             ylabel('Frequency (kHz)','Rotation', 90, 'position', [-23, 32.7]);                                         %% VJ2014 ,'Rotation', 90, 'position', [-20, 32.7]
			if strcmp(handles.screen_modus,'paper')
				par=get(hand,'parent');
...
% 			sig=getpart(sig,start_time,stop_time);
			...
			hand=plot(current_frame,str,myaxes2);                   % VJ2014  plot(current_frame,str);
...
%                 plot(time.*1000, val, 'r.');
                plot(time, val, 'r*'); % VJ 2015 the above line was original but this is correct
			end
		end
	end
