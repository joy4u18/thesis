...
smooth_sig= log_profile;%lowpass(log_profile,current_lowpass_frequency); % VJ2015 commented this to remvove filtering
...
for i=1:length(envpeaks);
    if envpeaks{i}.t>0.001 && envpeaks{i}.t<0.03 && envpeaks{i}.y> 0.01 % vj2014 actual for envpeaks{i}.t <= 0.02 and for envpeaks{i}.y is this 0.01 any other is my edition
        ep2{c}=ep{i};
        c=c+1;
    end
end
...
for i=1:length(envpeaks) % construct all maxima
    ...
    if (w>0  &&  (diffheight)>0.02);                             % Vj2014: actual formula was "w>0 && diffheight>0.02" any other is my modification
        envpeaks{i}.v2012_height_base_width_ratio=diffheight; % Vj2014: actual formula was "diffheight/w" any other is my modification
    else
