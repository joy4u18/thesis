    ...
    for channel_nr=1:nr_channels
        ...
        if min_time>0 && max_time <= siglen
            this_channel_part=getpart(this_channel,min_time,max_time);
                ...
                this_channel_part=getpart(this_channel,0,max_time); % VJ2015 original implemetation
%                                 this_channel_part=getpart(this_channel,0,2*maxdelay);         % VJ 2015 changed to compensate for the delay mistake in the original implementation
        ...
        corr=autocorrelate(this_channel_part,0,maxdelay); % VJ2014 normalization_mode changed see inside the function
        corvals=getvalues(corr);
        corvals=corvals(2:end)'; % skip the 0 delay and transpose
        
        
        saibuffer(channel_nr,:)=corvals./1;   % VJ2014 actual one is only: saibuffer(channel_nr,:)=corvals
    end
