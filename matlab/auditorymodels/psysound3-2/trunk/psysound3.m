...
if ~isempty(strfind(p, 'psysound3'))
    try
...
%     addpath(pwd);  % VJ2016 doing this manuallly as this takes more time
    
    %subdirs = {'GUI', 'AudioAnalysers', 'DataAnalysers', 'dataObjects', ...
    %           'dataStorage', 'utils'};
    
    subdir = genpath(pwd);  % VJ2016 doing this manuallly as this takes more time
    %% Do not add .svn paths
    
    %%% VJ2016 doing this manuallly as this takes more time
    
    
%     if isunix
%         colons = strfind(subdir,':');
%     elseif ispc
%         colons = strfind(subdir,';');
%     end
%     
...
%         if isempty(strfind(direc,'.svn')) && isempty(strfind(direc,'PsysoundData')) % Svn folders and PsySoundData folders are useless.
...
try
    ...
    fprintf('Setting data folder to: \n');
    disp([folderpath filesep 'PsysoundData']);
    fprintf('You can change this using File>>Preferences in the PsySound3 GUI. \n');
    prefs.showWaitBar= 0;
